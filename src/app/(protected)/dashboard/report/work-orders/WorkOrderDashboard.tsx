...
const WorkOrderDashboard = ({filter}: WorkOrderReportsProps) => {
  ...
  const fetchWorkOrders = async (pageIndex: number, pageSize: number) => {
    ...
    try {
      ...
      let apiUrl = `work-orders/?page=${
        pageIndex + 1
      }&page_size=${pageSize}`;

      if (filter) {
        apiUrl += `&review_status=${filter}`
      }
      ...
      const transformedData = responseData.map((apiOrder: any) => ({
        ...
        status: apiOrder.status
          ...
          : "Pending",
        isApproved: apiOrder.is_approved,
        reviewDate: apiOrder.review_date,
        reviewedBy: apiOrder.reviewed_by?.full_name || "NA",
        reviewNotes: apiOrder.review_notes,
        reviewStatus: apiOrder.review_status,
      }));
  ...
  const columns: ColumnDef<WorkOrder>[] = [
    {
      ...
      header: "Work Order",
      cell: ({ row }) => {
        const reviewStatus = row.original.reviewStatus || "pending_review";
        let reviewIcon, reviewClass, reviewLabel;

        switch (reviewStatus) {
          case "approved":
            reviewIcon = <FiCheckCircle className="text-green-500" />;
            reviewClass = "bg-green-100 text-green-800";
            reviewLabel = "Approved";
            break;
          case "rejected":
            reviewIcon = <FiAlertTriangle className="text-red-500" />;
            reviewClass = "bg-red-100 text-red-800";
            reviewLabel = "Rejected";
            break;
          case "pending_review":
            reviewIcon = <FiClock className="text-yellow-500" />;
            reviewClass = "bg-yellow-100 text-yellow-800";
            reviewLabel = "Pending Review";
            break;
          default:
            reviewIcon = null;
            reviewClass = "bg-gray-100 text-gray-800";
        ...
        return (
          <div className="flex flex-col font-montserrat">
            ...
            {/* Review Status Badge */}
            <div
              className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs ${reviewClass}`}
            >
              {reviewIcon}
      ...
      cell: ({ row }) => {
        ...
        const handleStatusChange = async (
          ...
          try {
            ...
            setWorkOrders((prev) =>
              prev.map((wo) =>
                wo.id === workOrderId ? { ...wo, status: currentStatus } : wo
              )
            );
          }
        };
        ...
        return (
          <Select
            ...
            value={statusOptions.find((opt) => opt.value === currentStatus)}
            valueRender={(items) => {
